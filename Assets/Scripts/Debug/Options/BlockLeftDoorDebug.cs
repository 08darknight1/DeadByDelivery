...
public class BlockLeftDoorDebug : DebugOption
{
    public BlockLeftDoorDebug(): base("BlockLeftDoorDebug"){}
    ...
    public override void OptionExec()
    {
        if(Car != null)
        {
            ...
            {
                if (Car.transform.GetChild(x).GetComponent<CarDoorController>() && Car.transform.GetChild(x).name.Contains("Left"))
                {
